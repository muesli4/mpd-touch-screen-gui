...
{
    return vbox({ { true, lv }
                , { false, hbox({ { false, std::make_shared<text_button>(left_label, left_callback) }
                                , { false, std::make_shared<text_button>("▲", [=](){ lv->scroll_up(); }) }
                                , { false, std::make_shared<text_button>("▼", [=](){ lv->scroll_down(); }) }
                                }, 5, true) }
                }, 5, false);
}
...
{
    return hbox({ std::make_shared<text_button>(left_label, left_callback)
                , std::make_shared<text_button>("▲", std::bind(lv, list_view::scroll_up))
                , std::make_shared<text_button>("▼", std::bind(lv, list_view::scroll_down))
                });
}
*/
